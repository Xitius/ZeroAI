...
class ProviderValidatorTest {
    ...
    inner class ClassifyProbeResult {
        ...
        fun `successful probe with 2 models returns Success containing count`() {
            ...
            assertTrue("Reachable \u2014 2 models listed" in success.details)
        }
        ...
        @Test
        @DisplayName("provider alias with empty model list returns Success with 0 models")
        fun `provider alias with empty model list returns Success with zero models`() {
            val result =
                ProviderValidator.classifyProbeResult(
                    providerId = "gemini",
                    probeResult = Result.success(emptyList()),
